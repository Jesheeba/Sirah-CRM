...
export default function WhatsAppClient({
  ...
  const [composing, setComposing] = useState<WaPrefill | null>(initialCompose ?? null);

  const q = search.trim().toLowerCase();
  const visible = rows.filter((r) => {
    if (tab !== "all" && r.direction !== tab) return false;
    if (q) {
      const hay = `${r.to_phone ?? ""} ${r.to_name ?? ""} ${r.from_email ?? ""} ${r.body ?? ""}`.toLowerCase();
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Link href="/whatsapp/templates" className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">
            Templates
          </Link>
      ...
      {!providerEnabled && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
          No WhatsApp provider configured — sending opens WhatsApp (wa.me) and logs the message here.
          Set <code className="rounded bg-amber-100 px-1">WHATSAPP_TOKEN</code> and{" "}
          <code className="rounded bg-amber-100 px-1">WHATSAPP_PHONE_ID</code> to enable direct send + receipts.
        </div>
      )}
            ...
            {visible.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-10 text-center text-slate-400">
                  {rows.length === 0 ? "No messages yet — start a conversation." : "No messages match."}
                </td>
              </tr>
            )}
            {visible.map((r) => (
              <tr key={r.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-slate-700">{r.to_name || r.to_phone || r.from_email || "—"}</td>
                <td className="px-4 py-3 max-w-xs truncate text-slate-600">{r.body || "—"}</td>
